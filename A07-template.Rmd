...
### Your code here
NewData <- Trips %>%
  mutate(tripduration = as.numeric(edate - sdate)) %>%
  filter(tripduration < 13000)

ggplot(NewData) +
  aes(x = client, y = tripduration) +
...
People seem to check bikes out more earlier in the year, with a decline as the years go on. There was a steep drop off around the 330 day mark, indicating that very few people rented bikes out on that day. It also seems that the renting service only began on day 275 of the year, as there are no bikes rented out before then.
...
My observations here..... 
The number of bikes rented out during the week seems to be generally the same regardless of day of the week. Sunday and Saturday are the days with the lowest number of rentals, with Thursday having the most.
...
My observations here..... 
There are very few rentals in the first few hours of the day. There is a sharp spike in rentals at around 6 hours into the day, and another one at around 17 hours. After 17 hours, there is a gentle taper off as the day ends.
...
My observations here..... 
The number of rentals does not vary much throughout the hour. The most rentals happen around the forty minute mark, though that is also where the most variation is, as there are valleys as well. It does not seem that the minute of hours impacts bike rentals.
...
My comments here.....
It seeps that registered clients take out rentals much more than casual clients. Clients rent bikes on the weekend more than the weekdays, while the inverse seems to be true for registered clients.
...
### My code here
#inner join  
RideDistances <- inner_join(Trips, PairDistances)

ggplot() +
  geom_density(data = PairDistances, mapping= aes(x = distance), color = "red") +
  geom_density(data = RideDistances, mapping = aes( x = RideDistances$distance), color = "blue") +
  theme_minimal()
```
...
'Simple' had the start station and 'Simple2' had the end station. Since StationPairs needed pairs of start and end stations, these variables had to have different names to represent their role in the pair. Otherwise, when joined, they would have been considered the same variable, and been merged into one column. 
